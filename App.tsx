...
import { LayoutGrid, Utensils, Dumbbell, Scale, History, FileDown, ShieldCheck, Settings, Wifi, WifiOff } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const score = useMemo(() => calculateDailyScore(currentLog, macros, 1950), [currentLog, macros]);
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'workout': return <WorkoutTracker log={currentLog} updateLog={updateLog} />;
      ...
      case 'settings': return (
        <div className="p-8 space-y-10 pb-32 bg-[#0f172a] min-h-screen text-white">
          ...
          <div className="stealth-card rounded-[32px] p-6 space-y-6">
            <div className="flex items-center gap-4 p-4 bg-blue-500/10 rounded-2xl border border-blue-400/20">
              <ShieldCheck className="text-blue-400" size={24} />
              <div>
                <h4 className="text-sm font-bold">Biometric Encryption</h4>
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">Device-Local Protocol Active</p>
              </div>
            </div>
            <div className="space-y-4">
              <button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-full py-4 bg-rose-500/10 text-rose-500 rounded-2xl font-black uppercase text-[10px] tracking-widest border border-rose-500/20">Purge Local Data</button>
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen relative bg-[#0f172a] no-scrollbar flex flex-col font-sans overflow-hidden">
      ...
      <div className="fixed bottom-6 left-6 right-6 z-50">
        <nav className="glass-nav rounded-[32px] p-2 flex justify-between shadow-2xl max-w-[380px] mx-auto">
